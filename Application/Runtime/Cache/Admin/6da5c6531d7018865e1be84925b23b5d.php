						...
						<div class="row-fluid">
							<div class="portlet box blue">
								<div class="portlet-title">
									<div class="caption"><i class="icon-edit"></i>产品系列</div>
									<div class="tools">
								...
								<div class="portlet-body">
									<div class="btn-group">
										<button class="btn green" onclick="javascript:window.location.href='<?php echo U('Admin/Product/classify_add');?>'">
									...
									<table class="table table-striped table-hover table-bordered" id="sample_editable_1">
										...
										<tbody>
											<?php if(is_array($data)): $i = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr class="" >
												<td><?php echo ($vo["product_classify_id"]); ?></td>
												<td><?php echo ($vo["sp"]); echo ($vo["title"]); ?></td>
												<td>
													<input type="text" class="span2" id="sort<?php echo ($key); ?>" value="<?php echo ($vo["sort"]); ?>" />
													<input type="button" onclick="changeValue('#sort<?php echo ($key); ?>','product_classify','product_classify_id',<?php echo ($vo["product_classify_id"]); ?>,'sort',$('#sort<?php echo ($key); ?>').val())" class="btn span2" value="修改" />
												</td>
												<td>
													<?php if($vo['status'] == 0): ?><a href="javascript:void(0);" onclick="changeStatus(this,'product_classify','product_classify_id',<?php echo ($vo["product_classify_id"]); ?>,'status',<?php echo ($vo['status']); ?>)" class="btn red icn-only"><i class="icon-remove-sign icon-white"></i></a>
													<?php else: ?>
														<a href="javascript:void(0);" onclick="changeStatus(this,'product_classify','product_classify_id',<?php echo ($vo["product_classify_id"]); ?>,'status',<?php echo ($vo['status']); ?>)" class="btn green icn-only"><i class="icon-ok-sign icon-white"></i></a><?php endif; ?>
												</td>
												<td><?php echo ($vo["createtime"]); ?></td>
												<td><?php echo ($vo["lastsavetime"]); ?></td>
												<td><a href="javascript:window.location.href='<?php echo U('Admin/Product/classify_editor',array('id'=>$vo["product_classify_id"]));?>'">编辑</a></td>
												<td><a onclick="del(this,'product_classify','product_classify_id',<?php echo ($vo["product_classify_id"]); ?>,1)">删除</a></td>
