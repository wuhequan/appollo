						...
						<div class="row-fluid">
							<div class="portlet box blue">
								<div class="portlet-title">
									<div class="caption"><i class="icon-edit"></i>光辉历程</div>
									<div class="tools">
								...
								<div class="portlet-body">
									<div class="btn-group">
										<button class="btn green" onclick="javascript:window.location.href='<?php echo U('Admin/About/info_add');?>'">
										...
										<tbody>
											<?php if(is_array($data)): $i = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr class="" >
												...
												<td>
													...
													<input type="button" onclick="changeValue('#sort<?php echo ($key); ?>','info','info_id',<?php echo ($vo["info_id"]); ?>,'sort',$('#sort<?php echo ($key); ?>').val())" class="btn span2" value="修改" />
												</td>
												
												<td>
													<?php if($vo['status'] == 0): ?><a href="javascript:;" onclick="changeStatus(this,'info','info_id',<?php echo ($vo["info_id"]); ?>,'status',<?php echo ($vo['status']); ?>)" class="btn red icn-only"><i class="icon-remove-sign icon-white"></i></a>
													<?php else: ?>
														<a href="javascript:;" onclick="changeStatus(this,'info','info_id',<?php echo ($vo["info_id"]); ?>,'status',<?php echo ($vo['status']); ?>)" class="btn green icn-only"><i class="icon-ok-sign icon-white"></i></a><?php endif; ?>
												</td>
												<td><a href="javascript:window.location.href='<?php echo U('Admin/About/info_editor',array('id'=>$vo["info_id"],'action'=>'Paper','tpl'=>'index_AE'));?>'">编辑</a></td>
												<td><a onclick="del(this,'info','info_id',<?php echo ($vo["info_id"]); ?>)">删除</a></td>
						...
						<!-- END DASHBOARD STATS -->
							...
							<form action="#" class="form-horizontal form-bordered">
								<div class="control-group">
									<label class="control-label">20年深厚的品牌积淀</label>
									<div class="controls">
										<textarea id="info" class="m-wrap span6" rows="6" ><?php echo ($datas["info"]); ?></textarea>
										<span class="help-inline">回车换行为一标题</span>
									</div>
								</div>
...
<!-- END BODY -->
	...
	<script type="text/javascript">
		...
		function submitClick(){
			...
			$.post(
				"<?php echo U('Admin/About/info1_Submit');?>",
				{
				...
				function(data){
					if(data==1){
		                success("<?php echo U('Admin/About/info');?>");
					}else{
						errorNo();
					}
				}
