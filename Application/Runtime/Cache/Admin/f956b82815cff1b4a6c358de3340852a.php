						...
						<div class="row-fluid">
							<div class="portlet box blue">
								...
								<div class="portlet-body">
									<div class="btn-group">
										<button class="btn green" onclick="javascript:window.location.href='<?php echo U('Admin/Public/add',array('action'=>'Admin','tpl'=>'admin_add'));?>'">
									...
									<table class="table table-striped table-hover table-bordered" id="sample_editable_1">
										...
										<tbody>
											<?php if(is_array($data)): $i = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr class="" >
												...
												<td>
													<?php if($vo['status'] == 0): ?><a href="#" onclick="changeStatus(this,'admin','admin_id',<?php echo ($vo["admin_id"]); ?>,'status',<?php echo ($vo['status']); ?>)" class="btn red icn-only"><i class="icon-remove-sign icon-white"></i></a>
													<?php else: ?>
														<a href="#" onclick="changeStatus(this,'admin','admin_id',<?php echo ($vo["admin_id"]); ?>,'status',<?php echo ($vo['status']); ?>)" class="btn green icn-only"><i class="icon-ok-sign icon-white"></i></a><?php endif; ?>
												</td>
												<td><?php echo ($vo["lastlogintime"]); ?></td>
												<!-- <td><a href="javascript:window.location.href='<?php echo U('Admin/Public/editor',array('id'=>$vo["admin_id"],'action'=>'Admin','tpl'=>'admin_editor'));?>'">编辑信息</a></td> -->
												<td><a href="javascript:window.location.href='<?php echo U('Admin/Admin/role',array('dataid'=>$vo["admin_id"]));?>'">编辑权限</a></td>
												<td><?php if($vo['admin_id'] != 1): ?><a onclick="del(this,'admin','admin_id',<?php echo ($vo["admin_id"]); ?>)">删除</a><?php endif; ?></td>
