...
class NewsController extends BaseController{
	...
	public function index(){
		//banner
		$banner=M('banner_news')->where('status=1')->order('sort desc')->find();
		...
		//新闻列表
		$pid=I('get.pid');
		$where['news_classify_id']=$pid;
		$where['status']=1;
		$arr=M('news')->where($where)->order('sort desc,showtime desc')->limit(6)->select();
	...
	public function news_sub(){
        ...
        if($first < $count){
        	...
        	$arr=M('news')->where($where)->order('sort desc,showtime desc')->limit($first,'6')->select();
	...
	public function video1(){
		...
		$curr_video=M('video1')->order('sort desc,showtime desc')->where('showindex=1')->find();
		$this->assign('curr_video',$curr_video);

		$where['video1_id']=array('neq',$curr_video['video1_id']);
		$where['status']=1;
		$arr=M('video1')->where($where)->order('sort desc,showtime desc')->limit(6)->select();
		foreach ($arr as $key => $value) {
			$arr[$key][sk]=$key+1;
		}
		$video=array();
		...
		$videos=M('video1')->where($where)->order('sort desc,showtime desc')->select();
	...
	public function index_sub(){
        ...
        $su=I('get.su');
        $first= '6'+ $su*6;
        $count=M('video1')->where($where)->count();
        if($first < $count){
        	$arr=M('video1')->where($where)->order('sort desc,showtime desc')->limit($first,'6')->select();
        	foreach ($arr as $k => $v) {
        		$arr[$k][sk]=$k+1;
        		$arr[$k]['showtime']=date('Y.m.d',$arr[$k]['showtime']);
        	}
